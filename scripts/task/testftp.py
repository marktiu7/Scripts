...
def get_dbname():
   try: 
        for db in dblist:
            cmd = "mysql -h{} -u{} -P11306 -p{} -N -e 'show databases;'| egrep -v 'mysql|db_helper|information_schema|performance_schema'".format(db,dbuser,dbpwd)
...
def backuptask(ip,edb):
  try:
    cmd = "mysqldump -h{} -u{} -P11306 -p{} -R -B {} | gzip > {}{}/{}.{}.sql.gz ".format(ip,dbuser,dbpwd,edb,uploadpath,datepath
     ,edb,datepath)
    command(cmd)
